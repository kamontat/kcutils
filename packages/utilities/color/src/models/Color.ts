import type { HEX } from "../typings/HEX";
import type { Named } from "../typings/Named";
import type { Type as NumberTypeString } from "../typings/NumberType";
import type { RGB } from "../typings/RGB";
import type { HSL } from "../typings/HSL";
import type { HSV } from "../typings/HSV";
...
export class Color extends WithLogger {
  ...
  constructor(rgb: RGB, loggerBuilder?: LoggerBuilder<"">) {
    ...
    super(
      loggerBuilder?.updateOption((b) =>
        b
          .withScopes(["color", id.toString(10)])
    ...
    if (validateRGB(rgb, false)) this.rgb = json.deepMerge({ a: 1 }, rgb);
      ...
      this.error = InvalidateColorError(
        this.getId().toString(),
    ...
    this.logger.print(
      "debug",
      `initiaize color object ${this.id} (valid = ${this.isValid()})`
    );
  }
  ...
  setRGB(rgb: Partial<RGB>): this {
    ...
    try {
      if (rgb.type !== "number")
        return this.setRGB(
          Object.assign(
            ...
            toType(
              "number",
              json.cleanObject(Object.assign({ type: "number" }, rgb)) as RGB,
              { min: 0, max: 255 }
            )
          )
        );
      ...
      const throwable = Throwable.from(e as Error, true);
  ...
  getBrightness(format: NumberTypeString = "number"): number {
    ...
    if (format === "number") return rounding(result, 3);
      ...
      return bound01(result, { min: 0, max: 255, digit: 2 });
    else if (format === "percent")
      return rounding(bound01(result, { min: 0, max: 255, digit: 4 }) * 100, 4);
  ...
  copyRGB(rgb: Partial<RGB>): Color {
    return new Color(
      Object.assign({}, this.toRGB(), rgb),
      LoggerBuilder.load(this.logger)
    );
  }
