...
describe("HSL to HSL", () => {
  ...
  test("enforceHSL with undefined value", () => {
    expect(enforceHSL(undefined)).toEqual(definedHSL());
  });

  test("convert unknown type object to default color object", () => {
    expect(hslToHsl(undefined as any, type.decimal)).toEqual(definedHSL());
    expect(hslToHsl(null as any, type.number)).toEqual(
      definedHSL({ type: type.number })
    );
  });

  test("convert empty type object will throw exception", () => {
    expect(hslToHsl({} as HSL, type.percent)).toEqual(
      definedHSL({ type: type.percent })
    );
  });

  test("change hsl with default type", () => {
    expect(hslToHsl({ type: "percent", h: 31, s: 12, l: 99 } as HSL)).toEqual(
  ...
  test.each([
    ...
    [
      { type: type.percent } as HSL,
      type.percent,
      definedHSL({ type: type.percent }),
    ],
  ])("convert HSL(%p, %s) to HSL(%p)", (hsl, type, result) => {
    expect(hslToHsl(hsl, type)).toEqual(result);
  });
  ...
  test.each([
    ...
    expect(roundedHSL(hsl)).toEqual(result);
  });

  test.each([
    [
      definedHSL({ a: 0.5948114, type: type.number }),
      3,
      definedHSL({ a: 0.594811, type: type.number }),
    ],
    [
      definedHSL({ a: 100, type: type.decimal }),
      3,
      definedHSL({ a: 1, type: type.decimal }),
    ],
    [
      definedHSL({
        a: 0.5123,
    ...
    expect(roundedHSL(hsl, digit)).toEqual(result);
  });
  ...
  test.each([
    ...
    [
      enforceHSL({ h: 0.25, s: 0.9, l: 0.49 }),
      enforceRGB({ r: 124.95, g: 237.41, b: 12.5 }),
    ],
    [
      enforceHSL({ h: 70, s: 20, l: 30, type: "percent" }),
      enforceRGB({ r: 67.32, g: 61.2, b: 91.8 }),
    ],

    [
      enforceHSL({ h: 300, s: 120, l: 100, a: 100, type: "number" }),
      enforceRGB({ r: 255, g: 255, b: 255, a: 1 }),
    ],
    [
      enforceHSL({ h: 0.95, s: 0.65, l: 0.6, a: 0.004 }),
      enforceRGB({ r: 219.3, g: 86.7, b: 126.48, a: 0.004 }),
    ],
