...
describe("HSV color", () => {
  ...
  test("enforceHSV with undefined value", () => {
    expect(enforceHSV(undefined)).toEqual(definedHSV());
  });

  test("convert unknown type object will return default value", () => {
    expect(hsvToHsv(undefined as any, type.decimal)).toEqual(
      enforceHSV({ type: type.decimal })
    );
  });

  test("convert empty type object will return default value", () => {
    expect(hsvToHsv({} as HSV, type.decimal)).toEqual(
      enforceHSV({ type: type.decimal })
    );
  });

  test("change hsv with default type", () => {
    expect(hsvToHsv({ type: "percent", h: 31, s: 12, v: 99 } as HSV)).toEqual(
  ...
  test.each([
    ...
    [
      { type: type.percent } as HSV,
      type.percent,
      definedHSV({ type: type.percent }),
    ],
  ])("convert HSV(%p, %s) to HSV(%p)", (hsv, type, result) => {
    expect(hsvToHsv(hsv, type)).toEqual(result);
  });
  ...
  test.each([
    ...
    expect(roundedHSV(hsv)).toEqual(result);
  });

  test.each([
    [
      definedHSV({ a: 0.59481, type: type.percent }),
      3,
      definedHSV({ a: 0.59481, type: type.percent }),
    ],
    [
      definedHSV({ a: 100, type: type.number }),
      3,
      definedHSV({ a: 1, type: type.number }),
    ],
    [
    ...
    expect(roundedHSV(hsv, digit)).toEqual(result);
  });
  ...
  test.each([
    ...
    [
      enforceHSV({ h: 0.25, s: 0.9, v: 0.49 }),
      enforceRGB({ r: 124.95, g: 40.61, b: 12.5 }),
    ],
    [
      enforceHSV({ h: 70, s: 20, v: 30, type: "percent" }),
      enforceRGB({ r: 76.5, g: 71.91, b: 61.2 }),
    ],

    [
      enforceHSV({ h: 300, s: 120, v: 100, a: 100, type: "number" }),
      enforceRGB({ r: 255, g: 211.65, b: 0, a: 1 }),
    ],
    [
      enforceHSV({ h: 0.95, s: 0.65, v: 0.6, a: 0.003 }),
      enforceRGB({ r: 153, g: 148.03, b: 53.55, a: 0.003 }),
    ],
