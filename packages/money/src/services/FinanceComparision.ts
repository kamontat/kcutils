...
import { Money, NumberMoney, PercentMoney } from "../models/money";
...
export class FinanceComparision<K extends string> extends SupportLogger {
  ...
  private asPercentage(diff: Money, base: Money): PercentMoney {
    return new PercentMoney(diff.divide(base.getAmount()).multiply(100).getAmount());
  }
}
